...
export default function Lightbox({
  ...
  return (
    <div
      ...
      aria-modal="true"
      aria-label={label}
      className={`fixed inset-0 z-[70] flex flex-col ${light ? "bg-[#F1EADA]" : "bg-[#584738]/[0.97]"}`}
      onClick={onClose}
    >
      <div
        className={`flex items-center justify-between gap-4 px-5 py-4 text-[10px] uppercase tracking-[0.24em] md:px-8 ${
          light ? "text-[#584738]/55" : "text-white/55"
        }`}
        onClick={(e) => e.stopPropagation()}
      >
        <span className="min-w-0 truncate">
          <span className={light ? "text-[#584738]" : "text-white"}>{String(index + 1).padStart(2, "0")}</span> /{" "}
        ...
        <button
          ...
          className={`flex h-10 w-10 shrink-0 items-center justify-center border transition-colors ${
            light ? "border-[#584738]/20 text-[#584738] hover:border-[#584738]" : "border-white/20 text-white hover:border-brand-gold hover:text-brand-gold"
          }`}
        >
          <X className="h-4 w-4" />
        </button>
          ...
          <>
            <button
              ...
              className={`absolute left-2 top-1/2 flex h-11 w-11 -translate-y-1/2 items-center justify-center border md:left-6 ${
                light ? "border-[#584738]/15 bg-white/70 text-[#584738]" : "border-white/15 bg-[#584738]/40 text-white hover:border-brand-gold"
              }`}
            >
