...
import { projects } from "@/data/avenue";

const PHONE = "919373632323";
...
function buildMessage(pathname: string): string {
  ...
  if (match) {
    ...
    if (project) {
      return `Hi, I'm interested in ${project.name} (${project.configuration}, Nashik). Could you please share more details?`;
    }
  }
  return "Hi, I'd like to know more about The Avenue developments in Nashik. Could you please assist me?";
}

export default function WhatsAppButton() {
  const pathname = usePathname();
  const href = `https://wa.me/${PHONE}?text=${encodeURIComponent(buildMessage(pathname))}`;
